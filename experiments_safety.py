...
def experiments(gamma, delta, epsilon, runs, mu1, mu2, C, safety_conf):
    ...
    while prop > safety_conf or len(bound) < 10:
        ...
        est = Estimator(gamma=gamma, delta=delta,
                        C=C,
                        enable_safety=True, safety_conf=safety_conf)
    ...
    plt.title('Proportion of executions which '
              + '''did not trigger the safety check
              '''
              + f'(m = {est.m}, n = {est.n}, '
              + gammaR + f' = {gamma}, ' + deltaR + f' = {delta})')
    plt.legend()
    plt.show()
