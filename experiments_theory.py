...
def experiments(gamma, delta, epsilon, runs, mu1, mu2):
    ...
    for i in indices:
        ...
        if prop >= delta and practical_n is None:
            ...
            while b - a > 2:
                k = (b + a)//2
                mean, std, prop = est.run_estimator(mechanism, mechanism_,
                                                    k, runs)
                if prop >= delta:
                    b = k
                else: a = k
    ...
    stds = np.array(stds)
    plt.rc('axes', titlesize=16, labelsize=12,
           titleweight='bold', labelweight='bold')
    plt.rc('figure', titlesize=18, titleweight='bold')
    fig = plt.figure(figsize=(20, 10))
    ax = fig.add_subplot(211)
    ax.axhline(y=epsilon, color='red',
               label=r"real $\epsilon$", linestyle='--')
    ax.axhspan(epsilon - gamma, epsilon + gamma, facecolor='orange', alpha=.2,
               label=r'$\gamma$-width band around $\epsilon$ '
               + '(required output)')
    # ax.axhline(y=epsilon + gamma, color='red', linestyle=':',
                # label=r'$\epsilon + \gamma$ and $\epsilon - \gamma$')
    # ax.axhline(y=epsilon - gamma, color='red', linestyle=':')
    ax.fill_between(nb, means - stds, means + stds, alpha=.2,
    ...
    ax.set_title(r'Estimated $\mathbf{\tilde{\epsilon}}$ '
    ...
    bar = ax2.bar([str(i) for i in nb], props, color='purple', alpha=.4)
    labels = [''] * len(props)
    ...
    ax2.set_title(r'Proportion of estimated $\mathbf{\tilde{\epsilon}}$ '
...
if __name__ == "__main__":
    ...
    mu1 = .3
    mu2 = .7
    # experiments(gamma, delta, epsilon, runs, mu1, mu2)
    runs = 100
    mu1 = 0
    mu2 = 1
    experiments(gamma, delta, epsilon, runs, mu1, mu2)
